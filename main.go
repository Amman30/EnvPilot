...
import (
	...
	"github.com/Amman30/EnvPilot/pilot"
)

func main() {
	
	pilot.Init(".env.example")

	port, err := pilot.Env.GetAsInt("PORT", 8080)
	if err != nil {
	...
	debug, err := pilot.Env.GetAsBool("DEBUG", false)
	if err != nil {
	...
	dbURL, err := pilot.Env.GetAsString("DATABASE_URL", "localhost")
	if err != nil {
	...
	threshold, err := pilot.Env.GetAsFloat("THRESHOLD", 0.1)
	if err != nil {
	...
	dbURLAny, err := pilot.Env.GetAsAny("DATABASE_URL", "string", "localhost")
	if err != nil {
		log.Printf("Error retrieving DATABASE_URL: %v", err)
	}
	...
	portNoDefault, err := pilot.Env.GetAsInt("PORT_NO_DEFAULT")
	if err != nil {
